...
public class Regel extends Welt{
	...
	public static void klicklinks(int x, int y) {
		...
		if(feld[xFeld][yFeld].getFeldhinbewegen()==true && wertWeiss==2 && (merkxwert-2)>=0 && (merkywert-2)>=0 &&
			...
			weissDamePruefen();
			return;
		}else if(feld[xFeld][yFeld].getFeldhinbewegen()==true && wertWeiss==2 && (merkxwert+2)<=7 && (merkywert-2)>=0 &&		
				feld[merkxwert+1][merkywert-1].getSchwarzSpieler()==true && feld[xFeld][yFeld]==feld[merkxwert+2][merkywert-2]) {		//Bedingung für Springen nach rechts bei weiss
			weissPlatzWechsel();
			weissSpringen();
		...
		if(feld[merkxwert][merkywert].getWeissWahl()==true && feld[xFeld][yFeld].getFeldhinbewegen()==true && wertWeiss==2) {			//Bedingung um weissen Stein zu setzen
			...
			wertSchwarz++;
			return;
		}else if(feld[merkxwert][merkywert].getWeissdameWahl()==true && feld[xFeld][yFeld].getFeldhinbewegen()==true && wertWeiss==2) {	//Bedingung um weissen Dame zu setzen
		...
		if(feld[xFeld][yFeld].getFeldhinbewegen()==true && wertSchwarz==2 && (merkxwert+2)<=7 && (merkywert+2)<=7 &&
				feld[merkxwert+1][merkywert+1].getWeissSpieler()==true && feld[xFeld][yFeld]==feld[merkxwert+2][merkywert+2]) {				//Bedingung für Springen nach rechts bei schwarz
			schwarzPlatzWechsel();
			schwarzSpringen();
			...
			schwarzDamePruefen();
			return;
		}else if(feld[xFeld][yFeld].getFeldhinbewegen()==true && wertSchwarz==2 && (merkxwert-2)>=0 && (merkywert+2)<=7 &&
				feld[merkxwert-1][merkywert+1].getWeissSpieler()==true && feld[xFeld][yFeld]==feld[merkxwert-2][merkywert+2]) {				//Bedingung für Springen nach links bei schwarz
			schwarzPlatzWechsel();
			schwarzSpringen();
	...
	public static void felderVorgabeOben() {																									//Prüft, die Felder, wo man mach oben gehen kann	
		...
		}else if((xFeld-1)>=0 &&(yFeld-1)>=0 && feld[xFeld-1][yFeld-1].getLeerFeld()==true) {													//prüft, ob man nach links oben gehen kann
			feld[xFeld-1][yFeld-1].setLeerFeld(false);
			feld[xFeld-1][yFeld-1].setFeldhinbewegen(true);
		}
		if((xFeld+2)<=7 && (yFeld-2)>=0 && feld[xFeld+1][yFeld-1].getSchwarzSpieler()==true && feld[xFeld+2][yFeld-2].getLeerFeld()==true) {	//prüft, ob man über einen Stein nach rechts oben springen kann
			feld[xFeld+2][yFeld-2].setLeerFeld(false);
			feld[xFeld+2][yFeld-2].setFeldhinbewegen(true);
		}else if((xFeld+1)<=7 && (yFeld-1)>=0 && feld[xFeld+1][yFeld-1].getLeerFeld()==true) {													//prüft, on man nach rechts oben gehen kann
			feld[xFeld+1][yFeld-1].setLeerFeld(false);
			feld[xFeld+1][yFeld-1].setFeldhinbewegen(true);
		}
	}
